module {
  ...
  ^bb1(%0: index):  // 2 preds: ^bb0, ^bb3
    %1 = memref.load %arg2[%0] {handshake.deps = #handshake<deps[<"store3" (2) [[0, 0]]>]>, handshake.name = "load5"} : memref<10xi32>
    cf.br ^bb2(%c0, %1 : index, i32) {handshake.name = "br1"}
  ^bb2(%2: index, %3: i32):  // 2 preds: ^bb1, ^bb2
    %4 = arith.muli %0, %c10 {handshake.name = "muli2"} : index
    %5 = arith.addi %2, %4 {handshake.name = "addi2"} : index
    %6 = memref.load %arg0[%5] {handshake.name = "load10"} : memref<100xi32>
    %7 = memref.load %arg1[%2] {handshake.deps = #handshake<deps[<"store2" (1) [[1, 9], [0, 0]]>, <"store2" (3) [[0, 0], [0, 0]]>]>, handshake.name = "load7"} : memref<10xi32>
    %8 = memref.load %arg4[%0] {handshake.name = "load8"} : memref<10xi32>
    %9 = arith.muli %8, %6 {handshake.name = "muli0"} : i32
    %10 = arith.addi %7, %9 {handshake.name = "addi0"} : i32
    memref.store %10, %arg1[%2] {handshake.deps = #handshake<deps[<"load7" (1) [[1, 9], [0, 0]]>, <"store2" (1) [[1, 9], [0, 0]]>]>, handshake.name = "store2"} : memref<10xi32>
    %11 = memref.load %arg3[%2] {handshake.name = "load9"} : memref<10xi32>
  ...
  ^bb3:  // pred: ^bb2
    memref.store %13, %arg2[%0] {handshake.name = "store3"} : memref<10xi32>
    %16 = arith.addi %0, %c1 {handshake.name = "addi4"} : index
    %17 = arith.cmpi ult, %16, %c10 {handshake.name = "cmpi1"} : index
