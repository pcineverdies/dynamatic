module {
  ...
  ^bb1(%0: index, %1: i32):  // 2 preds: ^bb0, ^bb3
    %2 = memref.load %arg2[%0] {handshake.deps = #handshake<deps[<"store3" (2) [[0, 0]]>]>, handshake.name = "load5"} : memref<10xi32>
    cf.br ^bb2(%c0, %2 : index, i32) {handshake.name = "br1"}
  ^bb2(%3: index, %4: i32):  // 2 preds: ^bb1, ^bb2
    %5 = arith.muli %0, %c10 {handshake.name = "muli2"} : index
    %6 = arith.addi %3, %5 {handshake.name = "addi2"} : index
    %7 = memref.load %arg0[%6] {handshake.name = "load10"} : memref<100xi32>
    %8 = memref.load %arg1[%3] {handshake.deps = #handshake<deps[<"store2" (1) [[1, 9], [0, 0]]>, <"store2" (3) [[0, 0], [0, 0]]>]>, handshake.name = "load7"} : memref<10xi32>
    %9 = memref.load %arg4[%0] {handshake.name = "load8"} : memref<10xi32>
    %10 = arith.muli %9, %7 {handshake.name = "muli0"} : i32
    %11 = arith.addi %8, %10 {handshake.name = "addi0"} : i32
    memref.store %11, %arg1[%3] {handshake.deps = #handshake<deps[<"load7" (1) [[1, 9], [0, 0]]>, <"store2" (1) [[1, 9], [0, 0]]>]>, handshake.name = "store2"} : memref<10xi32>
    %12 = memref.load %arg3[%3] {handshake.name = "load9"} : memref<10xi32>
  ...
  ^bb3:  // pred: ^bb2
    memref.store %14, %arg2[%0] {handshake.name = "store3"} : memref<10xi32>
    %17 = arith.addi %0, %c1 {handshake.name = "addi4"} : index
    %18 = arith.cmpi ult, %17, %c10 {handshake.name = "cmpi1"} : index
