module {
  ...
  ^bb2:  // pred: ^bb1
    ...
    %c0_i32_0 = arith.constant {handshake.name = "constant4"} 0 : i32
    %4 = memref.load %arg1[%1] {handshake.mem_interface = #handshake.mem_interface<MC>, handshake.name = "load5"} : memref<10xi32>
    %5 = arith.addi %1, %c1 {handshake.name = "addi1"} : index
    %6 = memref.load %arg1[%5] {handshake.mem_interface = #handshake.mem_interface<MC>, handshake.name = "load6"} : memref<10xi32>
    %7 = arith.index_cast %6 {handshake.name = "index_cast1"} : i32 to index
    %8 = arith.index_cast %4 {handshake.name = "index_cast2"} : i32 to index
    cf.br ^bb3(%8, %c0_i32_0 : index, i32) {handshake.name = "br1"}
  ...
  ^bb4:  // pred: ^bb3
    %c1_1 = arith.constant {handshake.name = "constant5"} 1 : index
    %12 = memref.load %arg2[%9] {handshake.mem_interface = #handshake.mem_interface<MC>, handshake.name = "load2"} : memref<10xi32>
    %13 = memref.load %arg3[%9] {handshake.mem_interface = #handshake.mem_interface<MC>, handshake.name = "load3"} : memref<10xi32>
    %14 = arith.index_cast %12 {handshake.name = "index_cast3"} : i32 to index
    %15 = memref.load %arg4[%14] {handshake.mem_interface = #handshake.mem_interface<MC>, handshake.name = "load4"} : memref<10xi32>
    %16 = arith.muli %13, %15 {handshake.name = "muli0"} : i32
    %17 = arith.addi %10, %16 {handshake.name = "addi0"} : i32
    %18 = arith.addi %9, %c1_1 {handshake.name = "addi2"} : index
    cf.br ^bb3(%18, %17 : index, i32) {handshake.name = "br2"}
  ^bb5:  // pred: ^bb3
    %c1_2 = arith.constant {handshake.name = "constant6"} 1 : index
    memref.store %10, %arg5[%1] {handshake.mem_interface = #handshake.mem_interface<MC>, handshake.name = "store1"} : memref<10xi32>
    %19 = arith.addi %1, %c1_2 {handshake.name = "addi3"} : index
